...
# Show that there are not any git changes
...
# Show that there are not any containers running
clear
echo "command: docker ps"
docker ps
read -r
echo "command: docker image ls"
docker image ls
echo "##################################"
read -r
...
# Show the Docker image build process
clear
echo "command: $dockerScript"
cd "$dockerDir" || echo "Could not change to $dockerDir"
$dockerScript
echo "##################################"
read -r

# Show that there is not previos slog data
clear
echo "command: ls -l data/slog-server"
ls -l data/slog-server
echo "##################################"
read -r

# Start docker-compose in the background
clear
echo "Start docker-compose another window so it can be shown"
echo "command: docker image ls"
docker image ls
echo "##################################"
read -r
clear
echo "Go to http://localhost:3000 and use admin/admin to login"
echo "##################################"
read -r
